...
async function downloadVideoFile(newVideo) {
  ...
  process.exec('node --experimental-wasm-threads utils/syntheticVideo.js',function (error, stdout, stderr) {
    ...
    resolve(`D:\\reptile\\8.16DownloadYoutubeVideoAndReprint\\files\\${new Date().Format("yyyy-MM-dd")}.mp4`)
  })
  return

  return new Promise((resolve, reject) => {
    Promise.all([downloadFile(VideoFormat.url, `newVideo.mp4`), downloadFile(audioFormats[0].url,`newAudio.mp3`)]).then(async () => {
      console.log('文件下载完毕')
      // 文件下载完毕
      process.exec('node --experimental-wasm-threads utils/syntheticVideo.js',function (error, stdout, stderr) {
        if (error !== null) {
...
exports.downloadVideoFile = downloadVideoFile
