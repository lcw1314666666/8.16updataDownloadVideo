...
async function download1080VideoFile(newVideo) {
  ...
  return new Promise((resolve, reject) => {
    Promise.all([downloadFile(VideoFormat.url, `newVideo${new Date().Format("yyyy-MM-dd")}.mp4`), downloadFile(audioFormats[0].url,`newAudio${new Date().Format("yyyy-MM-dd")}.mp3`)]).then(async () => {
      console.log('文件下载完毕')
      // 文件下载完毕
